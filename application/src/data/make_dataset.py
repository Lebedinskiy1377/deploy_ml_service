...
def merge_data(
    output: str, first_group_sku: str, second_group_sku: str, third_group_sku: str, promo: str, sku_dict: str
):
    """
    ...
    all_groups_sku_full = pd.merge(
        all_groups_sku_promo, sku_dict.rename(columns={"sku_id": "SKU"}), on="SKU", how="left"
    )

    all_groups_sku_full = all_groups_sku_full.sort_values(by="dates")

    all_groups_sku_full["week_num_expiration"] = (
        pd.to_datetime(all_groups_sku_full.expiration_date).dt.isocalendar().week
    )
